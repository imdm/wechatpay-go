...
func (a *OrdersApiService) QueryOrder(ctx context.Context, req QueryOrderRequest) (resp *QueryOrderResp, result *core.APIResult, err error) {
	var (
	...
	if req.SubMchid == nil {
		return nil, nil, fmt.Errorf("field `SubMchid` is required and must be specified in QueryOrderRequest")
	}
...
// 注意：
// - 调用分账接口后，需要解冻剩余资金时，调用本接口将剩余的分账金额全部解冻给二级商户
// - 此接口采用异步处理模式，即在接收到商户请求后，会先受理请求再异步处理，最终的分账结果可以通过查询分账接口获取
//
// 错误码列表
// |名称|状态码|描述示例|原因|解决方案
...
// |NO_AUTH|403|商户无权限|未开通分账权限|请开通商户号分账权限|
func (a *OrdersApiService) FinishOrder(ctx context.Context, req UnfreezeOrderRequest) (resp *FinishOrderResp, result *core.APIResult, err error) {
	var (
		localVarHTTPMethod   = nethttp.MethodPost
		localVarPostBody     interface{}
		localVarQueryParams  neturl.Values
	...
	localVarPath := consts.WechatPayAPIServer + "/v3/ecommerce/profitsharing/finish-order"
	// Make sure All Required Params are properly set
	...
	// Extract FinishOrderResp from Http Response
	resp = new(FinishOrderResp)
	err = core.UnMarshalResponse(result.Response, resp)
	if err != nil {
		return nil, result, err
	}
